document.addEventListener('DOMContentLoaded', function() {
   ...
   const burgerMenuSticks = document.querySelectorAll('.stick');
	...
	const bodyAddScrollLockButtons = document.querySelectorAll('.jsBodyAddScrollLock');
	const bodyRemoveScrollLockButtons = document.querySelectorAll('.jsBodyRemoveScrollLock');

	bodyAddScrollLockButtons.forEach(button => {
		button.addEventListener('click', () => {
			bodyAddScrollLock()
		})
	})

	bodyRemoveScrollLockButtons.forEach(button => {
		button.addEventListener('click', () => {
  ...
  // validate
  	function validateForms(selector, modalWindow) {
	...
	validateForms('#formMain', 'modalMainForm');
	validateForms('#formCall', 'modalMainCall');
	...
	serviceCardsForms.forEach(form => {
		...
		validateForms(formId, formModalWindowId);
	})

	// two cards centered
	let intViewportWidth = window.innerWidth;

	function cardsCentered() {
		servicesSliders.forEach((slider) => {
			if ((intViewportWidth > 1199) && (slider.querySelector('.splide__list').childElementCount == 2)) {
				slider.querySelector('.splide__track').style.display = 'flex';
				slider.querySelector('.splide__track').style.justifyContent = 'center';
			} else if ((intViewportWidth > 767) && (slider.querySelector('.splide__list').childElementCount == 1)) {
				slider.querySelector('.splide__track').style.display = 'flex';
				slider.querySelector('.splide__track').style.justifyContent = 'center';
			}
			else {
	...
	const equipmentSliderArrowPrev = equipmentSlider.querySelector('.splide__arrow-prev');
	const equipmentSliderArrowNext = equipmentSlider.querySelector('.splide__arrow-next');

	function equipmentSliderArrowsAlign() {
		equipmentSliderArrowPrev.style.bottom = `${equipmentSliderPhoto.offsetHeight / 2}px`;
		equipmentSliderArrowNext.style.bottom = `${equipmentSliderPhoto.offsetHeight / 2}px`;
	}
